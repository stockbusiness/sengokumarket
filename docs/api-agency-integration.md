...
- キー未設定・未指定・不一致のいずれも `401 Unauthorized`(`{"success": false, "message": "Unauthorized"}`)を返します。ただしAPIキー自体が本システム側で未設定(運用開始前)の場合のみ `503` を返します。
...
- エラーレスポンスは共通形式です(本システム内部向けAPIの`{error:{code,message}}`形式とは異なる、この連携APIファミリー専用の形式です)。

```json
{ "success": false, "message": "external_id is required" }
```
...
| `name` | string | ○ | 代理店名 |
| `parent_external_id` | string \| null | - | 親代理店の`external_id`。多階層ツリーを組む場合に指定。親がまだ登録されていない場合もエラーにはならず、未解決のまま保存し、親レコードが後から届いた時点で自動的に再紐付けします(詳細は7章)。未指定なら現状の親構成を維持(新規作成時は本部直下)。`null`または空文字`""`を指定すると本部直下に解除 |
...
|---|---|
| 401 | APIキー未指定・不一致 |
| 422 | `external_id`/`name`未指定、`default_commission_rate`が0〜100の範囲外、`status`不正、`login_email`の形式不正、自分自身または自分の配下代理店を親に指定(循環)など |
| 409 | `login_email`が既に他の管理者・代理店アカウントで使用されている |
| 500 | 本システム側の予期しないエラー |

いずれも本文は `{ "success": false, "message": "..." }` 形式です。

**注意:** `parent_external_id`に該当する代理店がまだ存在しない場合はエラーにはなりません(7章参照)。
...
一覧との違いは `child_external_ids`(直下の子代理店の`external_id`一覧)が含まれる点です。存在しない場合は`404`(`{"success": false, "message": "Agency not found"}`)。
...
- **`login_email`が本システムの既存の一般会員(評議員デジタル会員証の購入者等)のメールアドレスと一致する場合、新規アカウントは作成せず、その既存アカウントに代理店ポータルの権限を付与します。** この場合、既にパスワードを持っているため仮パスワードの発行・設定メールは送られず、代わりに「代理店ポータルが使えるようになった」ことを案内するメールのみ送信されます。ログインは今まで通り同じメールアドレス・パスワードで行えます。
  ...
  - 一致したメールアドレスが既に別の代理店ポータル・管理者アカウントとして使われている場合は`409`になります(上書き・付け替えは行いません)。
...
- 親代理店を先に登録するのが推奨ですが、**順序が前後してもエラーにはなりません。** 子代理店の`parent_external_id`が未登録の`external_id`を指す場合、本システムはその代理店を親なしの状態でいったん保存し、`parent_external_id`をそのまま覚えておきます。後から該当する`external_id`の代理店が登録されると、そのタイミングで自動的に再紐付けされます。
- 保存直後のレスポンスの`parent_external_id`には、実際にリンクできたかどうかに関わらず受け取った値がそのまま返ります(未解決の間もこの値は保持されます)。
- 自分自身を親に指定することはできません(`422`)。
- 自分の配下(子・孫…)の代理店を親に指定することもできません。循環構造になるため`422`を返します(これは対象の代理店が既に登録済みで、かつ実際に循環が生じる場合のみ判定されます)。
- 推奨する同期順序は変わらず「親代理店→子代理店→孫代理店…」ですが、前後してしまっても後追いで解決されます。
