...
| 対象 | 移行要否 | 内容 |
|---|---|---|
| AIアート教室の既存会員・予約・出欠・作品データ | **このシステムには該当なし** | 本書§35.1にある「AIアート教室の会員ID、予約・出欠・参加履歴、講座・会員プラン、作品・展示履歴」はAIアート教室側のデータであり、ショッピングシステム側の移行対象ではない。ただし7.3で述べた決済統合を行う場合、AIアート教室の既存有料会員の「決済済み状態」をこのシステムの`Order`としてどう表現するか(遡及作成 or 移行対象外とするか)は要確認 |
